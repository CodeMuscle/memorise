...
const Item = ({ id, label, onClick, icon: Icon, active, documentIcon, isSearch, level = 0, onExpand, expanded }: ItemProps) => {
  ...
  return (
    <div className={cn("group min-h-[27px] text-sm pr-3 py-1 w-full hover:bg-primary/5 flex items-center text-muted-foreground font-medium", active && 'bg-primary/5 text-primary')} onClick={onClick} role="button" style={{ paddingLeft: level ? `${(level * 12) + 12}px` : '12px' }}>
      {!!id && (
      ...
      {!!id && (
        <div className="ml-auto flex itens-center gap-x-2">
          <DropdownMenu>
          ...
          <div role="button"  onClick={onCreate} className="opacity-0 group-hover:opacity-100 h-full m-auto rounded-sm hover:bg-neutral-300 dark:hover:bg-neutral-600">
